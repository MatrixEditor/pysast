...
- ``pattern``: Contains information about the pattern to be matched.

- ``engine``:
    Specifies the pattern matching engine to be used. Currently, only regular
    expression (``re``) engine is supported.

- ``text``: The actual pattern text to be matched using the specified engine.
...
- ``meta``: Contains metadata information for the rule.
...
- ``title``: The title or name of the rule.
...
The ``rule-id`` should be unique within the rule set. The "input" property can is set
to "default" if no transformation is required on the input can can be left out. Specify
the desired pattern, pattern engine, and matching mode in the "pattern" section, where
the pattern engine and matching mode are optional. Provide a meaningful description,
severity, and title in the ``meta`` section. Optionally, add filter criteria in the
``filter`` section to narrow down the analysis scope of the rule.
